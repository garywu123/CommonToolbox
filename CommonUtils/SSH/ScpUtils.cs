...
namespace CommonUtils
{
	public static class ScpUtils
	{
        ...
        /// <param name="createDir">Create dir for the local file path if the directory of the local file doesn't exist.</param>
        ...
        /// <returns>return true for download file success.</returns>
        public static async Task<(int, List<(string, string)>)> DownloadFilesAsync(string host, int port, string user, string password,
                                                         ...
                                                         IProgress<int> progress = null)
        {
            ...
            using var client = await Task.Run(() => SshUtils.InitialSshClient(host, port, user, password));
            ...
            foreach (var remoteFile in remoteFiles)
            {
                ...
                {
                    failedFiles?.Add((remoteFile, ex.Message));
                }
            }
        ...
        public static async Task<(int, List<(string, string)>)> UploadFilesAsync(string host, int port, string user, string password,
            string[] localFiles, string remoteDir, IProgress<int> progress = null)
        {
            ...
            foreach (var localFile in localFiles)
            {
                ...
                {
                    failedFiles?.Add((localFile, ex.Message));
                }
            }
